    ...
    void unpack_yuy2(byte *const d[], const byte *s, int width, int height, int actual_size)
    {
        ...
        if (do_avx)
        {
            if (FORMAT == OB_FORMAT_Y8)
                unpack_yuy2_avx_y8(d, s, n);
            if (FORMAT == OB_FORMAT_Y16)
                unpack_yuy2_avx_y16(d, s, n);
            if (FORMAT == OB_FORMAT_RGB)
                unpack_yuy2_avx_rgb8(d, s, n);
            if (FORMAT == OB_FORMAT_RGBA)
                unpack_yuy2_avx_rgba8(d, s, n);
            if (FORMAT == OB_FORMAT_BGRA)
                unpack_yuy2_avx_bgr8(d, s, n);
            if (FORMAT == OB_FORMAT_BGRA)
                unpack_yuy2_avx_bgra8(d, s, n);
        }
        else
#endif
        {
...
#pragma omp parallel for
        ...
        for (; n; n -= 16, src += 32)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                int32_t t;
#define clamp(x) ((t = (x)) > 255 ? 255 : t < 0 ? 0 \
                                                : t)
...
#undef clamp
    ...
    void unpack_uyvyc(ob_format dst_format, rs2_stream dst_stream, byte *const d[], const byte *s, int w, int h, int actual_size)
    {
        switch (dst_format)
        {
        case OB_FORMAT_RGB:
